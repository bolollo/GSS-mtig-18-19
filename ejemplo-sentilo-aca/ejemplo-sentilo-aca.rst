...
#. Al final del código crear una función llamada *eachFeature* que recibe como parámetros un feature (elemento del GeoJSON) y un layer (elemento de Leaflet). La función sería la siguiente ::
...
#. Llamar a la API de la ACA para pedir la última lectura del sensor y así obtener la información. La url para obtener la última lectura es http://aca-web.gencat.cat/sentilo-catalog-web/component/map/EMBASSAMENT-EST/<id_sensor>. Por ejemplo ::

		http://aca-web.gencat.cat/sdim2/apirest/data/EMBASSAMENT-EST/080581-002-ANA006
...
#. Modificar nuestro proxy para obtener la información de un sensor. Escribir en nuestro archivo app.js justo debajo de la declaración de la variable *serverAca* ::
		...
		var serverAcaLastOb = 'http://aca-web.gencat.cat/sdim2/apirest/data/EMBASSAMENT-EST/';
...
#. Agregar justo antes del app.listen el código que nos va a ser de proxy. Copiar lo siguiente ::
		...
		app.all("/acalast/:id", function(req, res){
			...
			apiProxy.web(req, res, {
				target: serverAcaLastOb+req.params.id,
				changeOrigin: false, 
				ignorePath: true
			});
		});
...
#. Abrir la url http://localhost:3000/acalast/080581-002-ANA006 en el navegador para comprobar que el proxy está funcionando correctamente.
